...
import 'package:ferrero_asset_management/screens/assets/asset_capture_page.dart'; // Ensure this import is correct
...
class _ConsentFormPageState extends State<ConsentFormPage> {
  ...
  late Future<Shop?> _shopDetailsFuture;
  bool _isCompletedStatus = false; // State variable to track if status is 'Completed'

  // --- NEW/FIXED: Declare variables for pre-existing images and location ---
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            setState(() {
              ...
              _outletAddressController.text = shop.address;
              _VCTypeController.text = shop.vcType;
              _serialNumberController.text = shop.vcSerialNo.toString();
              _outletOwnerNameController.text = shop.contactPerson;
              _outletOwnerNumberController.text = shop.mobileNumber.toString();
              _stateController.text = shop.state;
              _postalCodeController.text = shop.postalCode.toString();
              // Set _isCompletedStatus based on fetched shop status
              _isCompletedStatus = shop.status.toLowerCase() == 'completed';

              // --- NEW/FIXED: Populate _preExistingImages and _preExistingLocation for prototype ---
              if (_isCompletedStatus) {
                // Assign your asset paths for prototype
                _preExistingImages = {
                  'outlet_exteriors_photo': 'assets/images/sample_outlet_exterior.jpg',
                  'asset_pics': 'assets/images/sample_asset_pic.jpg',
                  'outlet_owner_ids_pics': 'assets/images/sample_owner_id.jpg',
                  'outlet_owner_pic': 'assets/images/sample_owner_pic.jpg',
                  'serial_no_pic': 'assets/images/sample_serial_no.jpg',
                };
                _preExistingLocation = 'Lat: 28.6139, Lng: 77.2090 (Delhi)'; // Example hardcoded location
              } else {
                // Ensure they are empty/null for non-completed shops
                _preExistingImages = {};
  ...
  void _initiateEAgreement() {
    // Prevent action if status is completed
    if (_isCompletedStatus) {
      // For completed shops, just navigate directly, as fields are read-only
      // and we are simply showing existing data.
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => AssetCapturePage(
            ...
            username: widget.username,
            capturedImages: _preExistingImages,    // <--- FIXED: Pass _preExistingImages
            capturedLocation: _preExistingLocation, // <--- FIXED: Pass _preExistingLocation
            isShopCompleted: _isCompletedStatus,    // <--- FIXED: Pass _isCompletedStatus
          ),
        ),
      );
      return; // Exit here if shop is completed
    }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      CustomTextFormField(
                        ...
                        isFocusedNotifier: _outletUniqueCodeFocused,
                        validator: (value) => value == null || value.isEmpty ? 'Please enter UOC' : null,
                        readOnly: _isCompletedStatus, // Set readOnly based on status
                      ),
                      const SizedBox(height: 20),
                      CustomTextFormField(
                        controller: _outletAddressController,
