...
//
//   // ADDED: Set to store currently selected statuses for filtering
//   Set<String> _selectedStatuses = {};
//   // ADDED: List of all possible statuses (you might want to fetch these dynamically if they change)
...
//       setState(() {
//         _allShops = shops;
//         _filterShops(); // Apply initial filter (or show all if no statuses selected)
//       });
//     }).catchError((error) {
//       _showDialog('Error', 'Failed to load shops: $error');
//       setState(() {
...
//         final matchesSearch = shop.outletName.toLowerCase().contains(query);
//         final matchesStatus = _selectedStatuses.isEmpty ||
//             _selectedStatuses.contains(shop.status);
...
//     return Scaffold(
//       backgroundColor: const Color(0xFFFAF6EF),
//       // ADDED: AppBar for the hamburger menu
//       appBar: AppBar(
//         backgroundColor: const Color(0xFFFAF6EF),
//         elevation: 0,
//         leading: Builder(
...
//           },
//         ),
//         title: const Text('Shop Search', style: TextStyle(color: Colors.brown)),
//         centerTitle: true,
//         // Removed back button from here as it's now custom below the AppBar
//       ),
//       // ADDED: Drawer for status filtering
//       drawer: Drawer(
//         backgroundColor: const Color(0xFFFAF6EF),
//         child: Column(
//           children: [
...
//         children: [
//           Padding(
//             padding: const EdgeInsets.fromLTRB(20.0, 0.0, 20.0, 0.0), // Adjust padding as AppBar is now present
//             child: Column(
//               crossAxisAlignment: CrossAxisAlignment.start,
//               children: [
//                 // Moved back button functionality to AppBar leading icon
//                 // Retaining the line below the old back button for structure if needed, but adjusted padding.
//                 const SizedBox(height: 15), // Adjusted for spacing after AppBar
//                 ClipRect(
//                   child: BackdropFilter(
//                     filter: ImageFilter.blur(sigmaX: 5.0, sigmaY: 5.0),
//                     child: Container(
...
//                   return Center(child: Text('Error: ${snapshot.error}. Pull to refresh.'));
//                 } else if (!snapshot.hasData || snapshot.data!.isEmpty) {
//                   // MODIFIED: Text updated to reflect potential filtering
...
//                         Color? statusColor;
//                         switch (shop.status.toLowerCase()) {
//                           case 'completed':
//                             statusColor = Colors.green[700];
//                             break;
//                           case 'in progress':
...
import 'package:flutter/material.dart';
import 'dart:ui'; // Still needed if you use ImageFilter.blur elsewhere or want to re-add it
import 'package:ferrero_asset_management/screens/shops/consent_form_page.dart'; // Assuming this is correct
import 'package:ferrero_asset_management/models/shop_model.dart';
import 'package:ferrero_asset_management/services/app_api_service.dart';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        // No title property here, as requested
        // title: const Text('Shop Search', style: TextStyle(color: Colors.brown)), // REMOVED
        centerTitle: true,
      ),
      drawer: Drawer(
        backgroundColor: const Color(0xFFFAF6EF),
      ...
      body: Column(
        children: [
          // Adjusted padding to encompass only the search bar
          Padding(
            padding: const EdgeInsets.all(20.0), // Consistent padding around the search bar
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                // REMOVED: SizedBox(height: 15)
                // REMOVED: ClipRect with BackdropFilter (the blurred line)
                // REMOVED: SizedBox(height: 30)
